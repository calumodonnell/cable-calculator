...
app.controller('editCtrl', ['$scope', '$http', '$location', '$filter', 'connectors', function ($scope, $http, $location, $filter, connectors) {
    ...
    $scope.assembly_id = $location.search().assembly_id;

    localStorage.setItem('conn_1', '{"con_part_no":"' + $scope.cart[$scope.assembly_id].conn_1_part_no + '","con_series":"' + $scope.cart[$scope.assembly_id].conn_1_series + '","con_max_freq":"' + $scope.cart[$scope.assembly_id].conn_1_max_freq + '","con_rank":"' + $scope.cart[$scope.assembly_id].conn_1_rank + '","$$hashKey":"' + $scope.cart[$scope.assembly_id].conn_1_hash + '"}');
    localStorage.setItem('conn_2', '{"con_part_no":"' + $scope.cart[$scope.assembly_id].conn_2_part_no + '","con_series":"' + $scope.cart[$scope.assembly_id].conn_2_series + '","con_max_freq":"' + $scope.cart[$scope.assembly_id].conn_2_max_freq + '","con_rank":"' + $scope.cart[$scope.assembly_id].conn_2_rank + '","$$hashKey":"' + $scope.cart[$scope.assembly_id].conn_2_hash + '"}');

    $scope.conn_1 = JSON.parse(localStorage.getItem('conn_1'));
    $scope.conn_2 = JSON.parse(localStorage.getItem('conn_2'));

    $scope.droppedObjects1.push($scope.conn_1);
    $scope.droppedObjects2.push($scope.conn_2);

    $location.search();
    $scope.part_id = $location.search().part_id;

    $scope.search_freq = parseFloat($scope.cart[$scope.assembly_id].freq, 10);
    $scope.clength = parseInt($scope.cart[$scope.assembly_id].length, 10);
    ...
    $scope.saveCart = function (name, part_no, k1, k2, conn_1_price, conn_2_price, covering) {
        var cart = JSON.parse(localStorage.getItem('cart')) || [],
            conn_1,
            conn_1_part_no,
            conn_1_description,
